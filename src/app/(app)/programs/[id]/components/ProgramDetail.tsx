...
const ProgramDetail = ({ program }: ProgramDetailProps) => {
  ...
  const [imgSrc, setImgSrc] = useState(program.programImageUrl || fallbackProgram);
      ...
      <div className="bg-white/80 rounded-b-2xl shadow-xl p-8 flex flex-col relative">
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-6 mb-6">
          <Image
            ...
            width={80}
            height={80}
            className="w-20 h-20 rounded-full object-cover border-4 border-yellow-400 shadow-lg mx-auto md:mx-0"
          />
          <div className="flex-1 text-center md:text-left">
            <h1 className="text-3xl font-extrabold mb-1 text-gray-900">{program.title}</h1>
            <div className="font-semibold text-lg text-yellow-700 flex items-center justify-center md:justify-start gap-2">
              <svg className="w-5 h-5 text-yellow-500" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 11c0-1.657 1.343-3 3-3s3 1.343 3 3-1.343 3-3 3-3-1.343-3-3z" /></svg>
              {program.artisan?.name || "-"}
            </div>
            {program.artisan?.bio && <div className="text-sm text-gray-500 mt-1">{program.artisan.bio}</div>}
          </div>
        </div>
        {/* Divider */}
        ...
        <div className="flex flex-wrap items-center gap-3 mb-4">
          <span className="inline-flex items-center gap-1 bg-yellow-100 text-yellow-700 px-3 py-1 rounded-full text-xs font-semibold">
            <svg className="w-4 h-4 text-yellow-400" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 01-8 0" /></svg>
            {program.category?.name}
          </span>
          <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold ${program.isOpen ? "bg-green-100 text-green-700" : "bg-gray-200 text-gray-500"}`}>
            {program.isOpen ? (
        ...
        {/* Deskripsi */}
        <div className="mb-6">
          <p className="text-gray-800 text-base leading-relaxed whitespace-pre-line">{program.description}</p>
        </div>
        {/* Divider */}
        ...
        <div className="mb-2 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className="bg-yellow-50 rounded-xl p-4 flex flex-col items-center shadow-sm">
            <div className="font-medium text-gray-700 mb-1 flex items-center gap-1">
              <svg className="w-5 h-5 text-yellow-400" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
              Duration
            </div>
            <div className="text-gray-900 font-semibold">{program.duration}</div>
          </div>
          <div className="bg-yellow-50 rounded-xl p-4 flex flex-col items-center shadow-sm">
            <div className="font-medium text-gray-700 mb-1 flex items-center gap-1">
              <svg className="w-5 h-5 text-yellow-400" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M17.657 16.657L13.414 12.414a2 2 0 10-2.828 0l-4.243 4.243" /></svg>
              Location
            </div>
            <div className="text-gray-900 font-semibold">{program.location || "-"}</div>
          </div>
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="text-gray-900 font-semibold whitespace-pre-line bg-yellow-50 rounded-xl p-4 mt-1 shadow-sm">
            {program.criteria}
          </div>
        </div>
