...
interface IApplicantProfilePageProps {
    params: Promise<{ id: string }>;
}

export default async function ApplicantProfilePage({ params }: IApplicantProfilePageProps) {
    const { id } = await params;
    const profile = await applicantRepository.getApplicantProfile(id);
    if (!profile) {
        return (
